...
begin
     ...
     if (matrixsize = 1) then                               //размеры в модулях
     begin
          ...
          if (threadtype = 8) then                          //ширина ленты 144 диода на метр больше, чем расстояние между диодами
              width := ledwidth * 1.2
          else
              width := ledwidth * step;
     ...
     if (matrixsize = 2) then                               //размеры в сантиметрах
     begin
          if (RadioButtonVertical.Checked = true) then      //заносим вычисляемые значения размеров в соответствующие поля
          begin
